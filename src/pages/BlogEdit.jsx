...
const BlogEdit = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    unlayerFile: data?.unlayerFile,
    unlayerJson: data?.unlayerJson,
    hashTag: [{ name: "OK" }],
  });
  ...
  const handleSubmit = (e) => {
    ...
    // data.append("unlayerFile", formData.unlayerFile);
    // data.append("unlayerJson", formData.unlayerJson);
    data.append("hashTag", JSON.stringify(formData.hashTag));
    // console.log("Submitted data:", data);

    patch("/blogs", data).then((response) => {
      console.log(response);
      if (response?.status === 201) nav("/list");
    });
  };
